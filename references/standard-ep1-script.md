...
> is the format: the markers table, both `[DECK PACE:]` lines and where they sit, the
> demo density, the performance cues, and how fourth-grade sentences read out loud at
> this length.
...
> (the checklist beat) then workflow · THE END = close. Its format line names the
> retired three-layer screen base; today a script names ONE layer (deck, screen, or
> face) and never switches.

# Ep1 script · THE WINNING OFFER `[SLOT: the whole episode]`

**Format:** Screen-base. Claude Code desktop open the whole run, webcam bubble always
on. Full-face cuts marked. The finished card is on screen from second one.
...
The three floor markers are `[SCREEN ON:]`, `[COMMENT: @handle]`, and `[DEMO: exact
app, what is visible]`. Everything else in this table is episode-specific, legal
because this table defines it. Your script writes its own table the same way.

| Marker | Meaning |
|---|---|
| `[CARD]` | The finished one-page card on screen |
| `[SCREEN ON: ...]` | The live screen becomes the frame; covered beats go slide-free (the-presentation marker-precedence law) |
| `[CLAUDE CODE]` | The app, full frame |
| `[FULL FACE]` | Cut to the full face cam |
| `[COMMENT: @handle]` | The REAL comment, captured per the-presentation cage's real-captures law; a designed card only if the capture cannot be made |
| `[LABEL: word]` | Kinetic single-word flash, tied to a b-roll cut (edit-time only; unused in Ep1) |
| `[BOARD]` | The seven-gate scoreboard |
| `[GITHUB]` | The repo page as a beauty shot |
| `[DECK: name]` | Slide cut-in explaining the mechanism running right now (the-presentation cage) |
...
`[SCREEN ON: the real run playing behind these three lines; show each thing
...
> "in this video", no table of contents. The first `[DECK PACE:]` line sits at
> the top of the deck-carried act, and it is mandatory.
...
`[DECK PACE: the screen carries from here; slides only for flows, mechanisms, boards, and turn or proof moments, marked or not]`
...
`[CLAUDE CODE, full frame from here; bubble on]`
...
> The second `[DECK PACE:]` line sits at the top of the run, and it is mandatory
> too. the-presentation bounces a script missing either one.
...
> walkthrough uses the `[DEMO: screen, action]` line plus the spoken line and
...
Six alternate first lines, one take each, straight to camera. Not a separate
shoot: the Waterfall, the agent that turns one recording into thirty pieces, cuts
shorts from this recording, and these give it six openers.
...
format: a markers table that defines its own screen markers, the run's
`[DECK PACE:]` line at the top, one beat per human moment, a GUI demo instead of
the chat triplet. Read it for the shape only.

| Marker | Meaning |
|---|---|
| `[SCREEN ON: ...]` | The live screen becomes the frame |
| `[DEMO: exact app, what is visible]` | The screen this beat is staged on |
| `[FULL FACE]` | Cut to the full face cam |
...
`[DEMO: the accounting app, the un-reconciled trial balance, the red count in the corner]`
...
> `[DEMO:]` line names the screen, then the bulleted navigation steps give the
